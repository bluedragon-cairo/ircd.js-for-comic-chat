...
User.prototype = {
	...
	isHop(channel) {
		if(this.channelModes[channel.name])
			return this.channelModes[channel.name].match(/h/) || this.isOp(channel);
	},

	hop(channel) {
		this.channelModes[channel.name] += 'h';
	},

	dehop(channel) {
		if(this.channelModes[channel.name])
			this.channelModes[channel.name] = this.channelModes[channel.name].replace(/h/, '');
	},

	isVoiced(channel) {
		if(this.channelModes[channel.name])
			return this.channelModes[channel.name].match(/v/) || this.isHop(channel)|| this.isOp(channel);
	},

	voice(channel) {
		this.channelModes[channel.name] += 'v';
	},

	devoice(channel) {
		if(this.channelModes[channel.name])
			this.channelModes[channel.name] = this.channelModes[channel.name].replace(/v/, '');
	},
