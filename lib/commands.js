...
Commands.prototype = {
	...
	JOIN(user, channelNames, key) {
		...
		if(channelNames.startsWith('%'))
			channelNames = channelNames.replace('%', '');
		if(!channelNames.startsWith('#') && !channelNames.startsWith('&') && !channelNames.startsWith('!') && !channelNames.startsWith('+'))
	...
	_GETTOPIC(user, channelName) {
		const channel = this.server.channels.find(channelName);
		if(channel && channel.topic) {
			user.send(this.server.host, irc.reply.topic, user.nick, channel.name, ':' + channel.topic);
		} else {
			user.send(this.server.host, irc.reply.noTopic, user.nick, channelName, ':No topic is set');
		}
	},
		...
		if(userNames.length !== channelMasks.length) {
			...
			channelMasks.forEach(function(channelMask, i) {
				...
				if(!channel.findUserNamed(user.nick)) {
					...
					channel.send(':' + user.nick, 'KICK', channel.name, targetUser.nick, kickMessage);
	...
	TOPIC(user, channelName, topic) {
		...
		if(!channel) {
			...
			if(channel.modes.indexOf('t') === -1 || user.isHop(channel)) {
				channel.topic = topic;
				channel.send(':' + user.nick, 'TOPIC', channel.name, ':' + topic);
			} else {
	...
	PRIVMSG(user, target, message) {
		...
		if(!target || target.length === 0) {
			...
			if(!channel) {
				...
				if(this.server.config.onMessage)
					this.server.config.onMessage(user, channel, message);
			}
		} else {
	...
	MODE(user, target, modes, arg) {
		...
		if(target == 'ISIRCX')
			return user.send(this.server.host, '451', '*', ':You have not registered');
	...
	LIST(user, targets) {
		...
		for (var i in channels) {
			...
			if(channel.isPublic || channel.isMember(user)) {
				user.send(this.server.host, irc.reply.list, user.nick, channel.name, channel.memberCount || 1, ':[' + channel.modes + '] ' + channel.topic);
			}
		}

		user.send(this.server.host, irc.reply.listEnd, user.nick, ':End of /LIST command.');
	},
	...
	WHO(user, target) {
		...
		if(this.server.channelTarget(target)) {
				...
				channel.users.forEach(function(channelUser) {
					if(channelUser.isInvisible
						...
						user.send(server.host,
									...
									channelUser.hostname.replace('::ffff:', ''),
									server.config.hostname, // The IRC server rather than the network
									channelUser.channelNick(channel).replace('@', ''),
			...
			var matcher = this.server.normalizeName(target).replace(/\?/g, '.').replace(/\*/g, '.*');
			this.server.users.registered.forEach(function(targetUser) {
				...
				if(targetUser.isInvisible
					...
					user.send(server.host,
								...
								targetUser.username,
								targetUser.hostname.replace('::ffff:', ''),
